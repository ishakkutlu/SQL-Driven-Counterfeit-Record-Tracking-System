--------------------------------------------------
-- [UPDATE] Modify an existing counterfeit record
--------------------------------------------------
ALTER PROCEDURE [dbo].[udp_Sahtecilik_Guncelle] 
(
	@Rapor_Kodu int,
	@Rapor_Kodu_Yeni int,
	@Tespit nchar(10),
...
BEGIN
	BEGIN TRY
		...
		IF @Rapor_Kodu_Yeni<1
			RAISERROR ('Rapor kodu girilmemiş.', 16,10);
		IF (@Rapor_Kodu_Yeni != @Rapor_Kodu) AND (EXISTS(SELECT TOP 1 * FROM Sahtecilik_Raporu WHERE Rapor_Kodu=@Rapor_Kodu_Yeni))
		...
		UPDATE Sahtecilik_Raporu
		SET Rapor_Kodu = @Rapor_Kodu_Yeni, Tespit = @Tespit, Banknot_Cinsi = @Banknot_Cinsi, Banknot_Adedi = @Banknot_Adedi, 
		Nominal_Deger = @Nominal_Deger, Seri_Sira_No = @Seri_Sira_No, Rapor_Tarihi =@Rapor_Tarihi
		WHERE Rapor_Kodu=@Rapor_Kodu;

		COMMIT TRANSACTION TR01;
		SELECT 0 AS Sonuc, 'İşlem başarılı bir şekilde güncellendi.' AS Mesaj;
	END TRY
	BEGIN CATCH
